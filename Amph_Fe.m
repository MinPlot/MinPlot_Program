function APFU=Amph_Fe(D,W,O2_Ti)

% All ferrous iron, 23 O2 normalization (with correction for F and Cl)
[m,n]=size(D); %finds the x and y size of the input data matrix

%moles of cations
D_FeO=D;
D_FeO(:,5)=D(:,5)-D(:,5); %Makes Fe2O3=0
D_FeO(:,6)=D(:,6)+(D(:,5)./(159.6882./(2.*71.8444))); %Converts all Fe to FeO
