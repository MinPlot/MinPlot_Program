%%olivine Structural Formula with Fe2+ estimationn

function [StrctFrm, APFU]=olivine_fe2(data,headers,wantstrctfrm)
...
APFU(:,10)=sum(APFU,2); %calculations the total, which should be close to 3
...
%Si 
for c=1:m
    if APFU(c,1)<1.000 %if Si is <1.00, then the Si content is the APFU of Si
    StrctFrm(c,1)=APFU(c,1);
    else
    StrctFrm(c,1)=1; %or else, it assumes 1
    end
end
...
%Sum M
StrctFrm(:,12)=sum(StrctFrm(:,4:11),2);

%% Endmember calculation

if strcmp(wantstrctfrm, 'y')
    Endmembers(:,1)=APFU(:,8)./(APFU(:,6)+APFU(:,7)+APFU(:,8)+APFU(:,9)); % XFo
    Endmembers(:,2)=(APFU(:,6))./(APFU(:,6)+APFU(:,7)+APFU(:,8)+APFU(:,9)); % XFa
    Endmembers(:,3)=(APFU(:,7))./(APFU(:,6)+APFU(:,7)+APFU(:,8)+APFU(:,9)); % XTe
    Endmembers(:,4)=(APFU(:,9))./(APFU(:,6)+APFU(:,7)+APFU(:,8)+APFU(:,9)); % XCa-Ol
    ...
    StrctFrm=array2table(all,'VariableNames',{'Si_T','Al_T','Sum_T','Ti_M','Al_M','Cr_M','Ni_M','Fe2_M','Mn_M','Mg_M','Ca_M','Sum_M','XFo','XFa','XTep','XLrn'});  
    
else
    %outputs APFU table only
    APFU=array2table(APFU,'VariableNames',{'Si','Ti','Al','Cr','Ni','Fe2','Mn','Mg','Ca','Sum'});
end
